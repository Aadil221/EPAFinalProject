...
def test_post_question_as_admin():
    ...
    event = create_event(
        ...
        body={
            ...
            "category": "AWS",
            "difficulty": "Medium",
            "reference_answer": "Cloud platform"
        },
        groups="Admin"
    )
...
def test_post_question_no_groups():
    """Test POST question fails for users with no groups"""
    event = create_event(
        "POST",
        "/questions",
        body={"question_text": "What is AWS?", "category": "AWS", "difficulty": "Easy"},
...
def test_put_question_not_found():
    """Test PUT question fails when question doesn't exist"""
    mock_table.get_item.return_value = {}  # Empty response = not found
    ...
    assert "Question not found" in response["body"]


def test_put_question_no_fields_to_update():
    """Test PUT question fails when no valid fields provided"""
    mock_table.get_item.return_value = {
        "Item": {"id": "123", "question_text": "Old question"}
    }

    event = create_event(
        "PUT",
        "/questions/123",
        body={"invalid_field": "value"},  # No valid update fields
        groups="Admin"
    )
    ...
    assert response["statusCode"] == 400
    assert "No fields to update" in response["body"]
