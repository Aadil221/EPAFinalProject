...
1. **Least Privilege**
   Users, administrators, and services are granted only the permissions required to perform their intended functions. This reduces the impact of compromised credentials and limits the risk of unauthorised actions.

2. **Secure by Default**
   The system is designed so that secure behaviour is the default, not optional. Unauthenticated access is denied, administrative actions are restricted and sensitive operations require explicit authorisation.
...
4. **Separation of Responsibilities**
   Administrative, user, and development responsibilities are clearly separated. This reduces the risk of privilege escalation and limits the potential impact of insider threats or human error.

5. **Auditability and Accountability**
   Key actions, particularly administrative changes, are logged and traceable. This supports accountability, incident investigation, and non-repudiation.

6. **Automation and Consistency**
   Security-related tasks such as deployment, configuration, and validation are automated where possible. Automation reduces human error and ensures consistent enforcement of security controls across environments.

7. **Risk-Based Decision Making**
   Security decisions are made based on risk and business impact, rather than attempting to eliminate all possible threats. Controls are prioritised where they provide the greatest reduction in risk relative to complexity and cost.
...
| A10 | Changes are deployed via an automated CI/CD pipeline, rather than manual production updates. | Reduces human error, supports repeatability, and improves traceability of changes. | Protect pipeline access; use version control; CI/CD pipeline enforced via GitHub Actions with automated tests. |
| A11 | JWT tokens are stored in browser localStorage for user convenience. | This is an accepted risk for an internal tool where XSS prevention is the primary control. | Tokens are short-lived to limit exposure window; XSS prevention through React's default escaping and content sanitization. |
...
- Point-in-time recovery (PITR) enabled for data protection and recovery.
...
| /questions | POST | Cognito Authoriser + Admin Group | Create question (Admin only) | Privilege escalation (non-admin creates), injection, malicious content, XSS | Require "Admin" in cognito:groups claim, input validation & allowlists, size limits, content sanitization | Log creator (sub), new resource ID, group membership |
...
| /questions/{id} | PUT | Cognito Authoriser + Admin Group | Update question by ID (Admin only) | Mass assignment, injection, privilege escalation, XSS | Admin group check, whitelist updatable fields (question_text, category, difficulty, reference_answer), schema validation, content sanitization | Log updater (sub), changed fields metadata, group membership |
...
| A9 | IAM Roles and Policies | Lambda execution roles and service-to-service permissions. | Control what actions AWS services can perform on behalf of the application. |
...
An external threat actor with no legitimate access to the system, operating from the public internet. This actor may attempt to:
...
A legitimate user of the system with standard (non-admin) permissions. This actor may intentionally or unintentionally:
...
A legitimate administrator with permissions to manage interview questions. This actor may:
...
A threat actor with access to the AWS account hosting the service, either through misconfiguration, compromised credentials, or excessive permissions. This actor may:
...
A threat actor with access to the source code repository or CI/CD pipeline. This actor may:
...
| T14 | High | Malicious scripts injected via question content are executed in other users' browsers (XSS). | Tampering, Information Disclosure | Interview Question Data, Frontend, User Sessions | React provides default XSS protection through automatic escaping. Additional server-side content sanitization applied before storage. Input validation restricts allowed HTML/script tags. |
| T15 | Medium | S3 bucket containing frontend assets is accidentally configured for public access. | Information Disclosure | Application Frontend, S3 Bucket | S3 Block Public Access enabled at bucket level. CloudFront Origin Access Identity (OAI) restricts bucket access to CloudFront only. CDK context flag enforces secure defaults. |
| T16 | Medium | CloudFront does not return security headers, allowing clickjacking and content injection attacks. | Tampering, Information Disclosure | Application Frontend, User Sessions | CloudFront Response Headers Policy configured with: Content-Security-Policy, X-Frame-Options: DENY, X-Content-Type-Options: nosniff, Strict-Transport-Security. |
| T17 | High | DynamoDB table is accidentally deleted with no backup or recovery capability. | Tampering, Denial of Service | Interview Question Data, DynamoDB | Point-in-time recovery (PITR) enabled on DynamoDB table. Deletion protection enabled in production environment. Automated backups retained per organizational policy. |
...
| T14 | Cross-site scripting (XSS) attacks via malicious question content | Interview Question Data, Frontend | React's default escaping prevents XSS. Server-side sanitization of HTML/script tags before storage. Content Security Policy headers block inline scripts. | Implemented |
| T15 | Accidental public access to S3 bucket | S3 Bucket, Frontend Assets | S3 Block Public Access enabled. CloudFront OAI restricts bucket access. CDK secure defaults enforced via context flags. | Implemented |
| T16 | Missing security headers allow browser-based attacks | Frontend, CloudFront | CloudFront Response Headers Policy includes CSP, X-Frame-Options, X-Content-Type-Options, HSTS headers. | Implemented |
| T17 | DynamoDB table deletion without recovery capability | DynamoDB, Interview Question Data | Point-in-time recovery (PITR) enabled. Deletion protection enabled in production. Automated backup retention policy enforced. | Implemented |
...
| ST16 | XSS prevention through React escaping and sanitization | Create question with malicious content: `<script>alert('XSS')</script>` in question text | Content is escaped/sanitized and script does not execute when rendered | Manual | Passed |
| ST17 | CloudFront security headers configuration | Inspect HTTP response headers from CloudFront-served content | CSP, X-Frame-Options, X-Content-Type-Options, and HSTS headers present | Manual | Passed |
| ST18 | S3 bucket public access blocking | Attempt direct access to S3 bucket URL | Access denied; all public access blocked | Manual/Review | Passed |
| ST19 | CloudFront OAI restricts S3 access | Attempt to access S3 objects without going through CloudFront | Direct S3 access returns 403 Forbidden | Manual | Passed |
| ST20 | DynamoDB point-in-time recovery status | Verify PITR configuration in DynamoDB console or via AWS CLI | Point-in-time recovery is enabled on questions table | Manual/Review | Passed |
...
| 16 | Origin Access Identity (OAI) | An AWS CloudFront feature that restricts S3 bucket access to only come through CloudFront distribution. | S3 bucket policy allows GetObject only from CloudFront OAI. |
| 17 | Cross-Site Scripting (XSS) | A security vulnerability where malicious scripts are injected into web pages viewed by other users. | An attacker creates a question with `<script>` tags that execute in another user's browser. |
| 18 | Content Security Policy (CSP) | An HTTP header that controls what resources the browser is allowed to load, preventing XSS and injection attacks. | CSP header restricts inline scripts and only allows resources from trusted domains. |
| 19 | Point-in-Time Recovery (PITR) | A DynamoDB feature that provides continuous backups and allows restoration to any point within the retention period. | Restore DynamoDB table to its state from 2 hours ago after accidental deletion. |
...
**Changes from v1.1:**
- Added Assumption A11 (JWT token storage)
- Added Asset A9 (IAM Roles and Policies)
- Added Threats T14-T17 (XSS, S3 misconfiguration, security headers, DynamoDB deletion)
- Added corresponding mitigations for T14-T17
- Added Security Tests ST16-ST20
- Enhanced API table to include XSS threats and sanitization controls
- Added terms to glossary: OAI, XSS, CSP, PITR
